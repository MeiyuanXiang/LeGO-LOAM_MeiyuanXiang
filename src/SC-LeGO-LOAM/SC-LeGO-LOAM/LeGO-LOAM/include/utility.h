...
extern const bool useCloudRing = false; // 使用Ouster数据时需要改为false，if true, ang_res_y and ang_bottom are not used
...
// Ouster OS1-64
extern const int N_SCAN = 64;                               // 扫描线数
extern const int Horizon_SCAN = 1024;                       // 旋转一周采样次数，每线1024个点
extern const float ang_res_x = 360.0 / float(Horizon_SCAN); // 水平分辨率，水平方向每条线间隔0.35°
extern const float ang_res_y = 33.2 / float(N_SCAN - 1);    // 垂直分辨率，垂直方向每条线间隔0.53°
extern const float ang_bottom = 16.6 + 0.1;                 // 底部线束的角度偏移，垂直方向上起始角度是负角度，与水平方向相差16.7°
extern const int groundScanInd = 15;                        // 判断为地面的线数，以多少个扫描圈来表示地面

extern const bool loopClosureEnableFlag = true;   // 是否需要回环检测
...
extern const float historyKeyframeSearchRadius = 20.0; // 回环检测首尾距离，NOT used in Scan Context-based loop detector / default 7.0; key frame that is within n meters from current pose will be considerd for loop closure
extern const int historyKeyframeSearchNum = 25;        // 回环检测点云查找范围[-25，25]，2n+1 number of history key frames will be fused into a submap for loop closure
extern const float historyKeyframeFitnessScore = 1.5;  // icp匹配分数，平均距离小于1.5，the smaller the better alignment
