...
{
    // 行为ring，列为sector，生成ScanContext
    Eigen::MatrixXd sc = makeScancontext(_scan_down); // v1
    Eigen::MatrixXd ringkey = makeRingkeyFromScancontext(sc); // 取一个ring的均值
    Eigen::MatrixXd sectorkey = makeSectorkeyFromScancontext(sc); // 取同心扇形区的均值
...
std::pair<int, float> SCManager::detectLoopClosureID(void)
{
    ...
    std::pair<int, float> result{loop_id, yaw_diff_rad}; // 返回匹配最优的KeyFrame的ID和计算出来的角度差
